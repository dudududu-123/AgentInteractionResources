...
export class App {
  ...
  mount = async () => {
    ...
    //音声のセレクトボックス作成
    const selectBoxVoice = document.getElementById(
      "selectVoice"
    ...
    const modelSettings: MyModelSettings = JSON.parse(
      JSON.stringify(response.data)
    );
    //モーションのセレクトボックス作成
    const selectBoxMotion = document.getElementById(
      "selectMotion"
    ) as HTMLSelectElement;
    const motionNum: number = (
      modelSettings.FileReferences.Motions[
        "All" as keyof typeof modelSettings.FileReferences.Motions
    ...
    //表情のセレクトボックス作成
    const selectBoxExpression = document.getElementById(
      "selectExpression"
    ) as HTMLSelectElement;
    const expressionNum: number =
      modelSettings.FileReferences.Expressions.length;
    ...
    const expressionButton = document.getElementById(
      "setExpression"
    ) as HTMLElement;
  ...
  change_expression = (num: any) => {
    switch (num) {
      case -1:
        this.pixiCanvas?.hiyori.setExpression("hate");
        break;
      case 0:
        //                    this.pixiCanvas?.change_motion_dislike();
        this.pixiCanvas?.hiyori.setExpression("sad");
        break;
      case 1:
        //                    this.pixiCanvas?.change_motion_nomal();
        this.pixiCanvas?.hiyori.setExpression("nomal");
        break;
      case 2:
        //                    this.pixiCanvas?.change_motion_like();
        this.pixiCanvas?.hiyori.setExpression("like");
        break;
      case 3:
        //                    this.pixiCanvas?.change_motion_love();
        this.pixiCanvas?.hiyori.setExpression("love");
        break;
    }
  };
}
